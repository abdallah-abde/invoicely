...
import { getCanceledInvoices } from "@/features/invoices/db/invoice.query";
...
export default async function DashboardCanceledInvoicesPage() {
  ...
  const data = await getCanceledInvoices();

  const result = mapInvoicesToDTO(data);

  return (
    <InvoicesClient data={result} role={role} type={InvoiceCategory.CANCELED} />
  );
}
